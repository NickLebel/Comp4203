...
# Returns the average delay for all packets sent/received
...
# Graph the packet delivery ratio depending on the scenario by getting the packet delivery ratio 
#   for every experiment in the scenario, creating a table based on routing protocol/experiment results, then graphing that table
def graphPDR(scenario, title, xLabel, yLabel, xValues):
  data = {
    "AODV": [],
    "DSDV": [],
    "OLSR": []
  ...
  for protocol in data:
    for dirpath, dirnames, filenames in os.walk(f"{scenario}/{protocol}/xml"):
  ...
  print(df)
  print("\n\n")
  fig.savefig(f"./graphs/{scenario}-PDR")

# Graph the transmission delay depending on the scenario by getting the transmission delay
#   for every experiment in the scenario, creating a table based on routing protocol/experiment results, then graphing that table
def graphTransmissionDelay(scenario, title, xLabel, yLabel, xValues):
  data = {
    "AODV": [],
    "DSDV": [],
    "OLSR": []
  ...
  print(df)
  print("\n\n")
  fig.savefig(f"./graphs/{scenario}-transmission-delay")


# Graph the packet loss percentage depending on the scenario by getting the packet loss percentage 
#   for every experiment in the scenario, creating a table based on routing protocol/experiment results, then graphing that table
def graphPacketLoss(scenario, title, xLabel, yLabel, xValues):
  data = {
    "AODV": [],
    "DSDV": [],
    "OLSR": []
  ...
  print(df)
  print("\n\n")
  fig.savefig(f"./graphs/{scenario}-packet-loss")


# Graph the average throughput depending on the scenario by getting the average throughput
#   for every experiment in the scenario, creating a table based on routing protocol/experiment results, then graphing that table
def graphAvgThroughput(scenario, title, xLabel, yLabel, xValues):
  data = {
    "AODV": [],
    "DSDV": [],
    "OLSR": []
  ...
  for protocol in data:
    for dirpath, dirnames, filenames in os.walk(f"{scenario}/{protocol}/csv"):
      for filename in filenames:
        file = os.path.join(dirpath, filename)
  ...
  df = pd.DataFrame.from_dict(data)
  df.index = xValues
  fig = df.plot(kind="line", title=title, ylabel=yLabel, xlabel=xLabel, ylim=[10,25]).get_figure()
  print(title)
  print(df)
  print("\n\n")
  fig.savefig(f"./graphs/{scenario}-throughput")
